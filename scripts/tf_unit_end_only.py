...
def u_t(t):
    # t is a timestamp
    return 0.5*np.sin(20*2*np.pi*t)
...
if __name__ == "__main__":
    ...
    ui = tf_unit(u0/100, n_cells, t_array, truss.K, truss.M)

    file_name = os.path.join('data/dynamic/new_square/', 'l%d_m%.3f_tf_data_end_only.npz' %(layers, meshsize))
    np.savez(file_name, x=ui, t=t_array)

    print("Shape: " + str(np.shape(ui)))

    for i in range(len(ui)):
        plt.plot(t_array, ui[i])
